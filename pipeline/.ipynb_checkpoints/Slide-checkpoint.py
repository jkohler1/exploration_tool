...
class SlidePixplot:
    ...
    def CropSlideCoordRatios(self,x_size,y_size):  # remove extra replicates and crop to image
        ...
        if self.seg == False:
            ...
            self.seg_final, self.flag_seg_border_final = self.remove_seg_border(self.seg_triangle_open, imgray, bg_imgray_mean, bg_imgray_std, 3) #final semgentation
    ...
    def segmentation_pip(self,imgray):
        ...
        if self.npdi == True:
            print('ndpi True')
            regions = regions == 2
        self.seg_filled = binary_closing(regions, disk(self.structure_fill)) #fill the holes
        ...
        self.seg_filled_open_dilated = binary_dilation(self.seg_filled_open, disk(self.structure_dilation)) #image dilation
