...
config = {
    ...
    'id': id,
    'slide': None,
    'tile_phys_dir': os.path.join(current_folder, 'output_folder', f'tile_phys_dir_{id}'),  #tiles_physical_dir
    'tile_latent_dir': os.path.join(current_folder, 'output_folder', f'tile_latent_dir_{id}'),  #tiles_latent_dir
    'tile_size': 224,
    'tile_resize': 224,
    'seg': True,
    'ndpi': False,
...
def parse():
    ...
    parser.add_argument('--dino_model', '-dm', type=str, help='model name from dino', required=False)
    parser.add_argument('--dino2_model', '-d2m', type=str, help='model name from dino2', required=False)
    parser.add_argument('--clip_model', '-cm', type=str, help='model name from clip', required=False)
    parser.add_argument('--tile_phys_dir', '-tfd', type=str, default=config['tile_phys_dir'], help='path to tiles folder if wiling to keep tiles generated', required=False) #Physical directory
    parser.add_argument('--tile_latent_dir', '-tld', type=str, default=config['tile_latent_dir'], help='path to tiles folder if wiling to keep tiles generated', required=False) #Latent directory
